// ===========================================================================
// sketch_may26_r4_SEL_enHIGH -- COPY of sketch_may26_r4_SEL with the Senable
// polarity INVERTED to ACTIVE-HIGH (HIGH = enabled).
//
// WHY: the original drives Senable LOW during the shift (active-low). The
// datasheet prose says "retain Senable HIGH." This variant tests that reading:
// Senable is HIGH while clocking the 344 bits, LOW at idle/after. If the chip
// wanted active-high enable, the original (LOW-during-shift) would fail to
// clock and Sout would read all-0s -- exactly the symptom under investigation.
//
// The ONLY changes vs sketch_may26_r4_SEL are the three PIN_SENABLE writes
// (marked "// EN-HIGH"). Pins, timing, scan encoding, shifting, latching, the
// verification report, and the SEL/PING command parser are byte-identical.
// (Originals are never edited in place -- this is a new sketch folder.)
// ===========================================================================
...
void shiftScanChain(ShiftResult &r) {
  ...
  digitalWrite(PIN_SENABLE, HIGH);   // EN-HIGH: active-high enable (was LOW)
  ...
  digitalWrite(PIN_SENABLE, LOW);   // EN-HIGH: disabled = LOW (was HIGH)
...
void setup() {
  ...
  // Idle states
  digitalWrite(PIN_SENABLE, LOW);   // EN-HIGH: idle disabled = LOW (was HIGH)
  digitalWrite(PIN_SUPDATE, LOW);
  digitalWrite(PIN_SRST, LOW);
  digitalWrite(PIN_SCLKP, LOW);
  digitalWrite(PIN_SCLKN, LOW);
