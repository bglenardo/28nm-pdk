// Verification and scope code for ASIC scan chain
//
// SCOPE_MODE:
//   0 = Verification mode:
//       1032 clocks, reads/stores SOUT during final 344 clocks
//
//   1 = Full scope mode:
//       1032 clocks, no analogRead(), clean timing
//
//   2 = Chip 1 scope mode:
//       344 clocks, shifts pattern into chip 1, no analogRead()
//
//   3 = Chip 2 scope mode:
//       688 clocks, shifts pattern through chip 1 into chip 2,
//       no analogRead()
...
// Select the number of generated clocks based on SCOPE_MODE.
#if SCOPE_MODE == 2

const int TOTAL_CLOCKS = TOTAL_BITS;       // 344 clocks

#elif SCOPE_MODE == 3

const int TOTAL_CLOCKS = TOTAL_BITS * 2;   // 688 clocks

#else

const int TOTAL_CLOCKS = TOTAL_BITS * 3;   // 1032 clocks
...
void shiftScanChain(ShiftResult &r) {
  ...
  for (int cycle = 0; cycle < TOTAL_CLOCKS; cycle++) {
    ...
    // Read SOUT during the first 344 clocks.
    // Keep SCLKP HIGH while analogRead() occurs.
    if (cycle < TOTAL_BITS) {
    sout_log[cycle] = readSOUT_analog();
    }
  ...
  // Compare captured SOUT with transmitted SIN.
  ...
  // Scope modes do not read or verify SOUT.
  r.mismatches = -1;
  r.passed = false;
