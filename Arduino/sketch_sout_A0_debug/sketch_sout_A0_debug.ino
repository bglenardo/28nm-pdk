...
#define PIN_SOUT_ANALOG A0   // DEBUG: A1 -> A0 to test if Sout wire is on A0. NOT the canonical sketch.
...
void setup() {
  ...
  // PATCH (a): replaced timingTest(200) + configureDevice(1,0,14) + while(true){}
  // with a readiness banner. run_device_loop.py / test_scan_link.py sync on this.
  Serial.println(F("READY"));
}

// PATCH (b): command interface. Original loop() was empty.
//   PING        -> PONG
//   SEL f r c   -> configureDevice(f,r,c) [UNCHANGED] then SEL_DONE
//   <other>     -> ERR unknown cmd: <text>
void loop() {
  static String line;
